...
xkb_keysym_t CKeysymMaker::simpleKeysym(XIDeviceEvent* DeviceEvent) {
  ...
  if (effective_group >=
      (int)XkbDesc->map->key_sym_map[DeviceEvent->detail].group_info)
    effective_group =
        (int)XkbDesc->map->key_sym_map[DeviceEvent->detail].group_info - 1;
  int width = (int)XkbDesc->map->key_sym_map[DeviceEvent->detail].width;
  int effective_mods = DeviceEvent->mods.effective;
  int kt = (int)XkbDesc->map->key_sym_map[DeviceEvent->detail]
